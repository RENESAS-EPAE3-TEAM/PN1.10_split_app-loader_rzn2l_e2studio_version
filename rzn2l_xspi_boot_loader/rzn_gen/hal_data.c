...
static const xspi_qspi_extended_cfg_t g_qspi_ldr_extended_cfg =
		{ .unit = 0, .chip_select = XSPI_QSPI_CHIP_SELECT_0, .memory_size =
				XSPI_QSPI_MEMORY_SIZE_64MB, .p_timing_settings =
				&g_qspi_ldr_timing_settings,
#if 0 == 0
				.prefetch_en =
						(xspi_qspi_prefetch_function_t) XSPI_QSPI_CFG_UNIT_0_PREFETCH_FUNCTION,
#else
    .prefetch_en       = (xspi_qspi_prefetch_function_t) XSPI_QSPI_CFG_UNIT_1_PREFETCH_FUNCTION,
#endif
		};
const spi_flash_cfg_t g_qspi_ldr_cfg = { .spi_protocol =
		SPI_FLASH_PROTOCOL_1S_4S_4S, .address_bytes = SPI_FLASH_ADDRESS_BYTES_3,
		...
		.erase_command_list_length = sizeof(g_qspi_ldr_erase_command_list)
				/ sizeof(g_qspi_ldr_erase_command_list[0]), .p_extend =
				&g_qspi_ldr_extended_cfg, };
/** This structure encompasses everything that is needed to use an instance of this interface. */
const spi_flash_instance_t g_qspi_ldr = { .p_ctrl = &g_qspi_ldr_ctrl, .p_cfg =
		&g_qspi_ldr_cfg, .p_api = &g_spi_flash_on_xspi_qspi, };
...
static xspi_hyper_extended_cfg_t g_hyperbus0_extended_cfg =
		{ .unit = 0, .chip_select = XSPI_HYPER_CHIP_SELECT_1, .memory_size =
				XSPI_HYPER_MEMORY_SIZE_8MB, .data_latching_delay_clock = 0x08,
				.p_cs_timing_settings = &g_hyperbus0_cs_timing_settings,
				.p_autocalibration_preamble_pattern_addr = 0x00,
#if 0 == 0
				.prefetch_en =
						(xspi_hyper_prefetch_function_t) XSPI_HYPER_CFG_UNIT_0_PREFETCH_FUNCTION,
#else
    .prefetch_en                             = (xspi_hyper_prefetch_function_t) XSPI_HYPER_CFG_UNIT_1_PREFETCH_FUNCTION,
#endif
		};
const hyperbus_cfg_t g_hyperbus0_cfg = { .burst_type =
		HYPERBUS_BURST_TYPE_LINEAR, .access_space =
		HYPERBUS_SPACE_SELECT_MEMORY_SPACE, .read_latency_count =
		HYPERBUS_LATENCY_COUNT_6, .memory_write_latency_count =
		HYPERBUS_LATENCY_COUNT_6, .register_write_latency_count =
		HYPERBUS_LATENCY_COUNT_0, .p_extend = &g_hyperbus0_extended_cfg, };
/** This structure encompasses everything that is needed to use an instance of this interface. */
const hyperbus_instance_t g_hyperbus0 = { .p_ctrl = &g_hyperbus0_ctrl, .p_cfg =
		&g_hyperbus0_cfg, .p_api = &g_hyperbus_on_xspi_hyper, };
dmac_instance_ctrl_t g_transfer0_ctrl;

transfer_info_t g_transfer0_info = { .dest_addr_mode = TRANSFER_ADDR_MODE_FIXED,
		.src_addr_mode = TRANSFER_ADDR_MODE_INCREMENTED, .mode =
				TRANSFER_MODE_NORMAL, .p_dest = (void*) NULL, .p_src =
				(void const*) NULL, .length = 0, .src_size =
				TRANSFER_SIZE_1_BYTE, .dest_size = TRANSFER_SIZE_1_BYTE,
		.p_next1_src = (void const*) NULL, .p_next1_dest = (void*) NULL,
		.next1_length = 1 };
...
#endif
				.dmac_int_ipl = (12), .dmac_int_irq_detect_type = (0),

				.activation_source = ELC_EVENT_SCI3_TXI,

				.ack_mode = DMAC_ACK_MODE_BUS_CYCLE_MODE, .detection_mode =
						(dmac_detection_t)((0) << 2 | (1) << 1 | (0) << 0),
...
#if 0 == 0
				.channel_scheduling =
						(dmac_channel_scheduling_t) DMAC_CFG_CHANNEL_PRIORITY_UNIT0_CHANNEL0_7,
#else
    .channel_scheduling     = (dmac_channel_scheduling_t) DMAC_CFG_CHANNEL_PRIORITY_UNIT1_CHANNEL0_7,
#endif
				.p_callback = g_uart1_tx_transfer_callback, .p_context = NULL, };
...
/* Baud rate calculated with 0.000% error. */.baudrate_bits_b.abcse = 1,
		...
		.baudrate_bits_b.cks = 0, .baudrate_bits_b.brr = 1,
		.baudrate_bits_b.mddr = (uint8_t) 240, .baudrate_bits_b.brme = true };
...
const uart_cfg_t g_uart1_cfg = { .channel = 3, .data_bits = UART_DATA_BITS_8,
		.parity = UART_PARITY_OFF, .stop_bits = UART_STOP_BITS_1, .p_callback =
				plsw_uart_callback, .p_context = NULL, .p_extend =
				&g_uart1_cfg_extend, .p_transfer_tx = g_uart1_P_TRANSFER_TX,
		.p_transfer_rx = g_uart1_P_TRANSFER_RX, .rxi_ipl = (10),
...
const spi_flash_cfg_t g_qspi0_cfg = { .spi_protocol =
		...
		.dummy_clocks = SPI_FLASH_DUMMY_CLOCKS_0, .read_command = 0x03,
		.page_program_command = 0x02, .write_enable_command = 0x06,
		.status_command = 0x05, .write_status_bit = 0,
		.xip_enter_command = 0x20, .xip_exit_command = 0xFF,
...
const cmtw_extended_cfg_t g_timer0_extend = { .toc0 = CMTW_OUTPUT_PIN_RETAIN,
		.toc1 = CMTW_OUTPUT_PIN_RETAIN, .toc0_control =
				CMTW_OUTPUT_CONTROL_DISABLED, .toc1_control =
				CMTW_OUTPUT_CONTROL_DISABLED, .capture_ic0_source =
				CMTW_SOURCE_EDGE_RISING, .capture_ic1_source =
				CMTW_SOURCE_EDGE_RISING, .ic0_control =
				CMTW_INPUT_CONTROL_DISABLED, .ic1_control =
				CMTW_INPUT_CONTROL_DISABLED, .clear_source =
				CMTW_CLEAR_SOURCE_COMPARE_MATCH_CMWCOR, .counter_size =
...
/* Actual period: 0.01048576 seconds. */.period_counts = (uint32_t) 0x10000,
		.source_div = (timer_source_div_t) 3, .channel = 0, .p_callback = NULL,
...
const timer_instance_t g_timer0 = { .p_ctrl = &g_timer0_ctrl, .p_cfg =
		&g_timer0_cfg, .p_api = &g_timer_on_cmtw };
